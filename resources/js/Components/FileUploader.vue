<template>
    ...
    >
      ...
      <ul class="mt-4 lg:grid grid-cols-3 text-center" v-if="this.filelist.length" v-cloak>
        <li v-for="(file, index) in filelist" :key="index" class="text-xs text-blue-500 p-1">
          {{ file.name }}
          <button
            type="button"
  ...
  export default {
    ...
    methods: {
      ...
      dragover(event) {
        ...
        if (!event.currentTarget.classList.contains("bg-blue-100")) {
          event.currentTarget.classList.remove("bg-gray-100");
          event.currentTarget.classList.add("bg-blue-100");
          event.currentTarget.classList.add("border-blue-300");
        }
      },
      dragleave(event) {
        // Clean up
        ...
        event.currentTarget.classList.remove("border-blue-300");
      },
      drop(event) {
        event.preventDefault();
